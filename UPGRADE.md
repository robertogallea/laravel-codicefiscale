...
| `->getYear(): ?string`, `->getMonth(): ?string`, `->getDay(): ?string` | `ParsedCodiceFiscale::birthYear(): ?int`, `->birthMonth(): int`, `->birthDay(): int` | Real integers. `birthYear()` is nullable in agreement with `birthDate()` - both are null when reference-date resolution finds no plausible century (see README's "Reference-date resolution"). `birthDate()` gives you the assembled date directly in most cases. |
...
| 2.x | 3.x | Notes |
|---|---|---|
| `CityDecoderInterface` (`getList(): array<code, name>`) | `Contracts\BirthPlaceRepository` (`find(BirthPlaceCode, ?DateTimeImmutable $on = null): ?BirthPlace`, `existedEver(BirthPlaceCode): bool`, `search(string $name, ?DateTimeImmutable $on = null, ?int $limit = null): list<BirthPlace>`) | A repository with real dates, not a flat lookup table. |
| `CodiceFiscale::generate(..., $place, ...)` accepting *either* a cadastral code or a city name (resolved via `array_search()` against the decoder's list) | `Generator::generate()` accepts only a `BirthPlaceCode` on `Person::$birthPlace` - no reverse name lookup happens implicitly | If you only have a name, resolve it first with `$repository->search($name)`, pick the right candidate from the results, and pass its `->code()`. Generation itself never guesses from a name - see `docs/adr/0008-birthplace-repository-gains-core-name-search.md`. |
